...
class Minesweeper:
    ...
    def reveal(self, x, y):
        if not (0 <= x < self.width and 0 <= y < self.height):  # Check bounds
        ...
        if (y * self.width + x) in self.mines:  # Hit a mine
            return False
        
        self.revealed[y][x] = True
        self.revealed_cells += 1

        if self.count_mines_nearby(x, y) == 0:  # If no nearby mines, reveal adjacent cells
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    nx, ny = x + dx, y + dy
